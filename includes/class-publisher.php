...
class HashPoster_Publisher {
    ...
    private function get_featured_image_url( $post_id ) {
        ...
        // Intelligent Fallback: If no Featured Image is explicitly set (e.g., RSS auto-posters),
        // rapidly scrape the raw post content for the first embedded HTML image.
        $post = get_post( $post_id );
        if ( $post && preg_match( '/<img[^>]+src=[\'"]([^\'"]+)[\'"][^>]*>/i', $post->post_content, $matches ) ) {
            $img_url = $matches[1];
            // Ensure relative paths from auto-posters become absolute URLs
            if ( strpos( $img_url, 'http://' ) !== 0 && strpos( $img_url, 'https://' ) !== 0 && strpos( $img_url, '//' ) !== 0 ) {
    ...
    private function get_first_paragraph( $post ) {
        ...
        if ( ! empty( $matches[1] ) ) {
            ...
            // Map line breaks into real text newlines before scrubbing the rest of the HTML
            $first_paragraph = str_ireplace( array('<br>', '<br />', '<br/>'), "\n", $first_paragraph );
            $first_paragraph = strip_tags( $first_paragraph );
            $first_paragraph = trim( $first_paragraph );
            ...
            if ( str_word_count( $first_paragraph ) < 20 ) {
                ...
                if ( ! empty( $all_matches[1] ) ) {
                    ...
                    $combined = implode( "\n\n", $paragraphs );
                    $combined = str_ireplace( array('<br>', '<br />', '<br/>'), "\n", $combined );
                    $first_paragraph = strip_tags( $combined );
                    $first_paragraph = trim( $first_paragraph );
                }
            }
    ...
    private function get_limited_excerpt( $post, $word_limit = 200 ) {
        // Priority 1: Use explicit WP Excerpt field if defined
        $excerpt = get_the_excerpt( $post );
        
        if ( empty( $excerpt ) ) {
            // Priority 2: Fall back to full content, but safely convert structural HTML
            // blocks to plaintext newlines BEFORE stripping tags so paragraphs survive formatting
            $content = get_the_content( null, false, $post );
            
            // Ensure implicit formatting is explicitly wrapped in paragraph tags
            $content = wpautop( $content );
            
            // Convert breaks to literal newlines safely
            $content = str_ireplace( array('<br>', '<br />', '<br/>'), "\n", $content );
            $content = str_ireplace( '</p>', "\n\n", $content );
            
            // Mute WordPress caption shortcodes entirely to keep text clean
            $content = strip_shortcodes( $content );
