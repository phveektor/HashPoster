...
class HashPoster_OAuth_Handler {
    private $platforms = array( 'linkedin', 'bluesky', 'facebook' );
    ...
    public function is_connected( $platform ) {
        ...
        // For Telegram which uses direct bot token and channel ID instead of OAuth
        if ( $platform === 'telegram' ) {
            $api_credentials = get_option( 'hashposter_api_credentials', array() );
            return !empty( $api_credentials['telegram']['bot_token'] ) && !empty( $api_credentials['telegram']['channel_id'] );
        }

        // For Facebook, check OAuth tokens
        if ( $platform === 'facebook' ) {
            return !empty( $tokens['facebook'] ) && !empty( $tokens['facebook']['access_token'] ) && !empty( $tokens['facebook']['page_id'] );
        }

        // For all OAuth platforms (X, LinkedIn), check for access_token
    ...
    public function disconnect( $platform ) {
        ...
        // For Facebook, also clear API credentials (app_id, app_secret)
        if ( $platform === 'facebook' ) {
            $all_creds = get_option( 'hashposter_api_credentials', array() );
            unset( $all_creds[ $platform ] );
            update_option( 'hashposter_api_credentials', $all_creds );
        }
    }
    ...
    public function ajax_oauth_disconnect() {
        // Log basic request info (do NOT log tokens)
        error_log('[HashPoster DEBUG] ajax_oauth_disconnect called. nonce_present: ' . ( isset($_REQUEST['nonce']) ? 'YES' : 'NO' ) . ', platform: ' . ( $_REQUEST['platform'] ?? 'none' ) . ', user_id: ' . get_current_user_id() );

        check_ajax_referer( 'hashposter_oauth', 'nonce' );
        error_log('[HashPoster DEBUG] ajax_oauth_disconnect: nonce verification passed.');
        
        if ( ! current_user_can( 'manage_options' ) ) {
            error_log('[HashPoster DEBUG] ajax_oauth_disconnect: insufficient permissions for user ' . get_current_user_id() );
            wp_die( 'Insufficient permissions' );
        }

        $platform = sanitize_text_field( $_POST['platform'] ?? '' );
        if ( ! in_array( $platform, $this->platforms ) ) {
            error_log('[HashPoster DEBUG] ajax_oauth_disconnect: invalid platform: ' . $platform );
            wp_die( 'Invalid platform' );
        }

        // Remove tokens
        $this->disconnect( $platform );
        error_log('[HashPoster DEBUG] ajax_oauth_disconnect: oauth tokens removed for ' . $platform );

        // Also clear manual credentials (e.g., Facebook Page token, Bluesky app password)
        $option_name = 'hashposter_api_credentials';
        $api_credentials = get_option( $option_name, array() );
        error_log('[HashPoster DEBUG] ajax_oauth_disconnect: before unset - ' . $platform . ' credentials keys: ' . ( isset($api_credentials[$platform]) ? implode(', ', array_keys($api_credentials[$platform])) : 'NONE' ) );
        
        if ( isset( $api_credentials[ $platform ] ) ) {
            // Completely remove the platform credentials
            unset( $api_credentials[ $platform ] );

            // Write option atomically.
            // This avoids issues when the wp_options table contains duplicate rows for the same option_name
            // and also ensures object-cache sees a clean value.
            $deleted = delete_option( $option_name );
            $added = add_option( $option_name, $api_credentials, '', 'yes' );

            // Clear option caches (especially important when autoloaded via alloptions)
            wp_cache_delete( $option_name, 'options' );
            wp_cache_delete( 'alloptions', 'options' );
            wp_cache_delete( 'notoptions', 'options' );

            // SQLite/persistent object cache: flush to ensure admin UI reflects change immediately
            if ( function_exists( 'wp_cache_flush' ) ) {
                wp_cache_flush();
            }

            $verify = get_option( $option_name, array() );
            $still_exists = ( is_array( $verify ) && isset( $verify[ $platform ] ) );
            error_log('[HashPoster DEBUG] ajax_oauth_disconnect: api_credentials rewrite - delete_ok: ' . ( $deleted ? 'YES' : 'NO' ) . ', add_ok: ' . ( $added ? 'YES' : 'NO' ) );
            error_log('[HashPoster DEBUG] ajax_oauth_disconnect: verification (get_option) - ' . $platform . ' still exists? ' . ( $still_exists ? 'YES' : 'NO' ) );
        } else {
            error_log('[HashPoster DEBUG] ajax_oauth_disconnect: no manual credentials found for ' . $platform );
        }
    ...
    private function get_facebook_oauth_url( $callback_url ) {
        ...
        return 'https://www.facebook.com/v24.0/dialog/oauth?' . http_build_query( $params );
    }
    ...
    private function handle_facebook_callback() {
        ...
        if ( isset( $body['access_token'] ) ) {
            // Get user's pages
            $pages_url = 'https://graph.facebook.com/v24.0/me/accounts?access_token=' . $body['access_token'];
